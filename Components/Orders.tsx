    ...
    <View style={{backgroundColor:'rgb(245, 245, 240)'}}>
      ...
      <ScrollView>
        ...
        <View style={{ alignItems:'center',marginBottom:150}}>
        {
          cartItems.map((item) =>{
            return(
            <View key={item.id} style={{ marginTop:20, width:350, flexDirection:'row', height:150, alignItems:'center', justifyContent:'space-around', elevation:2, backgroundColor:'white', borderRadius:5}}>
              <View>
              ...
              <Text style={{color:'rgb(0, 204, 0)', fontWeight:'800', marginTop:10, }}>₹{item.price * item.quantity}</Text>
              </View>
              <View style={{flexDirection:'row', alignItems:'center', justifyContent:'space-around', width:130, marginLeft:35, borderRadius:10,backgroundColor:'rgb(221, 255, 204)',elevation:2}}>
                <TouchableOpacity onPress={()=>decreaseQuantity(item.id)}><Text style={{ textAlign:'center', fontWeight:'400', color:'black', fontSize:25, width:40}}>-</Text></TouchableOpacity>
                <Text style={{width:30, height:25,color:"rgb(0, 204, 0)", textAlign:'center'}}>{item.quantity}</Text>
                <TouchableOpacity onPress={()=>handleIncreaseQuantity(item.id)}><Text style={{ textAlign:'center', fontWeight:'400', color:'black', fontSize:20, width:40}}>+</Text></TouchableOpacity>
              </View>
              </View>
            </View>
            )
